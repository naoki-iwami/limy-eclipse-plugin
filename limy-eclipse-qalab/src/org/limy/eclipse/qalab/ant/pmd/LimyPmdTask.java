...
public class LimyPmdTask extends Task {

    private Path classpath;
    private Path auxClasspath;
    private List<LimyFormatter> formatters = new ArrayList<LimyFormatter>();
    ...
    public static void main(String[] args) {
        LimyPmdTask task = new LimyPmdTask();
        task.setShortFilenames(true);
        task.setTargetJDK("1.5");
        task.setEncoding("UTF-8");
        task.setRuleSetFiles("D:/var/prog/eclipse-plugin/latest/limy-eclipse-qalab/resource/pmd-ruleset.xml");
        task.setSupportUTF8(true);
        LimyFormatter formatter = new LimyFormatter();
        formatter.setType("xml");
        formatter.setToFile(new File("C:/var/workspace/eclipse/runtime-EclipseApplication/javatest/dest/pmd_report.xml"));
        task.addFormatter(formatter);
        FileSet fileset = new FileSet();
        fileset.setDir(new File("C:/var/workspace/eclipse/runtime-EclipseApplication/javatest/dest/src"));
        fileset.setIncludes("**/*.java");
        task.addFileset(fileset);
        
        task.setProject(new Project());
        task.execute();
        //     encoding="${src.encoding}" rulesetfiles="${pmd.config}" supportUTF8="true">
        //<formatter type="xml" toFile="${dest.dir}/pmd_report.xml"/>
        //<fileset dir="${all.src.dir}" includes="**/*.java"/>
      //</pmd>

    }
    public void setSupportUTF8(boolean value) {
        if (value) {
    ...
    public void addFormatter(LimyFormatter f) {
        formatters.add(f);
    }
    ...
    private void doTask(){
        ...
        for (LimyFormatter formatter: formatters) {
    ...
    private void validate() throws BuildException {
        ...
        for (LimyFormatter f: formatters) {
